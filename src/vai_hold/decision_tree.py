...
DERIVE_LEAVES: list[tuple[str, str, str]] = [
    ...
    ("A2-16", "transfer_in_flight", "SmmHold memo transfer 未查驗完"),
    ...
    ("A2-16", "smm_memo_transfer", "Defect 片增加 → transferHold 累積 memo"),
    ("A2-05", "ai_incomplete", "有防守但 AI 未齊"),
    ("A2-06", "ai_result_invalid", "AI 結果無效"),
    ("A2-07", "ai_ok", "整批 OK → 申請解除"),
    ("A2-08", "defect_hold_handoff", "有 Defect 且 SMM Hold 已接手"),
    ("A2-09", "defect_without_formal_hold", "有 Defect 但 SMM Hold 未確認"),
...
LEAF_FACT: dict[tuple[str, str], str] = {
    ...
    ("A2-16", "transfer_in_flight"): "in_flight=TRANSFER_HOLD:*",
    ("A1-09", "retry_same_action"): "action_state=RETRY_WAIT",
    ("A2-16", "smm_memo_transfer"): "SmmHold memo 未涵蓋已知 Defect slot",
    ...
    ("A2-07", "ai_ok"): "ai_status=COMPLETE_OK",
    ("A2-08", "defect_hold_handoff"): "COMPLETE_DEFECT 且 has_defect_hold",
    ("A2-09", "defect_without_formal_hold"): "COMPLETE_DEFECT 且無正式 Hold",
...
def _fact_caption(rule_id: str, reason: str, facts: dict) -> str:
    mapping = {
        ...
        ("A2-07", "ai_ok"): f"ai_status={facts.get('ai_status')}",
        ("A2-08", "defect_hold_handoff"): f"has_defect_hold={facts.get('has_defect_hold')}",
        ("A2-09", "defect_without_formal_hold"): f"has_defect_hold={facts.get('has_defect_hold')}",
