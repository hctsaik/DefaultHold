# Evidence：`transfer_retry_transient`

**情境：** transferHold 暫時拒絕：同一命令最多 3 次
...
- `transfer.intent` `CHECK_AI_SCAN_COMPLETE` rule=`A2-16` receipt=`None`
- `transfer.sent` `CHECK_AI_SCAN_COMPLETE` rule=`A2-16` receipt=`None`
- `transfer.receipt` `CHECK_AI_SCAN_COMPLETE` rule=`A2-16` receipt=`REJECTED` attempt_no=1
- `transfer.sent` `CHECK_AI_SCAN_COMPLETE` rule=`A1-09` receipt=`None`
- `transfer.receipt` `CHECK_AI_SCAN_COMPLETE` rule=`A1-09` receipt=`REJECTED` attempt_no=2
- `transfer.sent` `CHECK_AI_SCAN_COMPLETE` rule=`A1-09` receipt=`None`
- `transfer.receipt` `CHECK_AI_SCAN_COMPLETE` rule=`A1-09` receipt=`REJECTED` attempt_no=3
- `incident.opened` `CHECK_AI_SCAN_COMPLETE` rule=`A2-18` receipt=`None`
...
```json
{
  ...
  "OrderDb": {
    "OrderId": "20260101-000000008-EQP01",
    "Lifecycle": "OPEN",
    "WorkState": "NEED_HOLD",
...
### EVAL 6 `CHECK_AI_SCAN_COMPLETE`

- **DECISION** `A2-16` / `smm_memo_transfer` action=`TRANSFER_HOLD`
...
- **DECISION** `A1-09` / `retry_same_action` action=`TRANSFER_HOLD`
...
### EVAL 12 `CHECK_AI_SCAN_COMPLETE`
